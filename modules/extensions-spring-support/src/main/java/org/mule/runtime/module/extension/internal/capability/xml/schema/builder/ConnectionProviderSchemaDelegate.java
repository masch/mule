...
final class ConnectionProviderSchemaDelegate {
  ...
  void registerConnectionProviderElement(ConnectionProviderModel providerModel, DslElementSyntax elementSyntax) {
    ...
    providerModel.getParameterGroupModels().stream()
        .forEach(g -> builder.addParameterGroupsToSequence(
                                                           builder.registerParameters(providerType, g.getParameterModels()),
                                                           sequence));
